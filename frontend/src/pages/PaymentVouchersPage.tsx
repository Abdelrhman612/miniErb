...
import { paymentVoucherService, treasuryService, supplierService, customerService, accountService } from '../services/api';
import type { PaymentVoucherResponseDto, CreatePaymentVoucherDto, UpdatePaymentVoucherDto, TreasuryResponseDto, Supplier, Customer, Account } from '../types';
import { VoucherStatus } from '../types';

interface PaymentFormProps {
    initial?: PaymentVoucherResponseDto;
    treasuries: TreasuryResponseDto[];
    suppliers: Supplier[];
    customers: Customer[];
    accounts: Account[];
    onSave: (dto: CreatePaymentVoucherDto) => Promise<void>;
    onCancel: () => void;
    saving: boolean;
}

function PaymentForm({ initial, treasuries, suppliers, customers, accounts, onSave, onCancel, saving }: PaymentFormProps) {
    const [treasuryId, setTreasuryId] = useState<number>(initial?.treasuryId ?? (treasuries[0]?.id ?? 0));
    const [partyType, setPartyType] = useState<'supplier' | 'customer' | 'account' | 'other'>(
        initial?.supplierId ? 'supplier' : initial?.customerId ? 'customer' : initial?.counterAccountId ? 'account' : 'other'
    );
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">الخزنة (دائن) *</label>
                <select
                    value={treasuryId}
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-semibold text-slate-400 mb-1">نوع الطرف المستفيد *</label>
                    <select
                        value={partyType}
                    ...
                    >
                        ...
                        <option value="account">حساب من دليل الحسابات (مصروفات، إلخ)</option>
            ...
            {partyType === 'account' && (
                <div>
                    <label className="block text-xs font-semibold text-slate-400 mb-1">الحساب (مدين - مصروفات / أصول) *</label>
                    <select
            ...
            {partyType === 'other' && (
                <div>
                    <label className="block text-xs font-semibold text-slate-400 mb-1">اسم الطرف (المسدد له) *</label>
                    <input
                        type="text"
                        value={partyName}
            ...
            <div>
                ...
                <input
                    ...
                    disabled={saving}
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 font-bold text-rose-400"
                />
            </div>
...
function PaymentVoucherDetailsModal({
    voucher,
    onClose,
}: {
    voucher: PaymentVoucherResponseDto | null;
    onClose: () => void;
}) {
    if (!voucher) return null;

    return (
        <Modal isOpen={!!voucher} onClose={onClose} title={`تفاصيل سند صرف: ${voucher.voucherNumber}`}>
            <div className="print-container bg-slate-900 print:bg-white text-slate-100 print:text-slate-900 p-6 rounded-2xl space-y-6">
                <div className="border-b border-slate-800 print:border-slate-300 pb-4 flex justify-between items-start">
                    <div>
                        <h2 className="text-xl font-black">سند صرف (Payment Voucher)</h2>
                ...
                <div className="grid grid-cols-2 gap-4 bg-slate-950/60 print:bg-slate-100 p-4 rounded-xl border border-slate-800 print:border-slate-300 text-sm">
                    ...
                    <div>
                        <span className="text-slate-400 print:text-slate-600 block text-xs">المدفوع إلى (الطرف)</span>
                        <span className="font-bold text-rose-300 print:text-rose-900">{voucher.resolvedPartyName}</span>
                    </div>
                    <div>
                        <span className="text-slate-400 print:text-slate-600 block text-xs">المبلغ</span>
                        <span className="font-black text-lg text-rose-400 print:text-rose-900">
...
export function PaymentVouchersPage() {
    const [vouchers, setVouchers] = useState<PaymentVoucherResponseDto[]>([]);
    ...
    const [editTarget, setEditTarget] = useState<PaymentVoucherResponseDto | null>(null);
    const [saving, setSaving] = useState(false);
    const [saveError, setSaveError] = useState('');

    const [detailsTarget, setDetailsTarget] = useState<PaymentVoucherResponseDto | null>(null);
    const [confirmTarget, setConfirmTarget] = useState<PaymentVoucherResponseDto | null>(null);
    const [cancelTarget, setCancelTarget] = useState<PaymentVoucherResponseDto | null>(null);
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            setError('');
            const [vData, tData, sData, cData, aData] = await Promise.all([
            ...
            setError('فشل في تحميل بيانات سندات الصرف.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const openEdit = (v: PaymentVoucherResponseDto) => {
    ...
    const handleSave = async (dto: CreatePaymentVoucherDto) => {
        setSaving(true);
        setSaveError('');
        try {
            if (editTarget) {
                await paymentVoucherService.update(editTarget.id, dto as UpdatePaymentVoucherDto);
            } else {
                await paymentVoucherService.create(dto);
            }
            closeModal();
            await loadData();
        } catch (err) {
            if (axios.isAxiosError(err)) {
                setSaveError(err.response?.data?.message ?? 'فشل في حفظ سند الصرف.');
            } else {
                setSaveError('فشل في حفظ سند الصرف.');
            }
        } finally {
            setSaving(false);
        }
    ...
    const handleConfirm = async () => {
        ...
        try {
            await paymentVoucherService.confirm(confirmTarget.id);
    ...
    const handleCancel = async () => {
        ...
        setActionLoading(true);
        try {
            await paymentVoucherService.cancel(cancelTarget.id);
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto text-right" dir="rtl">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-black text-slate-100">سندات الصرف (Payment Vouchers)</h1>
                    <p className="text-slate-500 text-sm mt-1">إدراج وإدارة سندات صرف الأموال من الخزنة وحسابات الموردين والعملاء</p>
                </div>
                <button
                    onClick={openCreate}
                ...
                <div className="grid gap-4">
                    {filtered.length === 0 ? (
                        <div className="text-center py-16 bg-slate-900/30 border border-slate-800/60 rounded-2xl">
                            <p className="text-slate-500">لا توجد سندات صرف مطابقة لخيارات البحث أو التصفية.</p>
                        </div>
                    ) : (
                        filtered.map(v => (
                            <div
                            ...
                            >
                                ...
                                <div className="flex flex-wrap items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-3 md:pt-0 border-slate-800">
                                    <div className="text-left">
                                        ...
                                        <p className="text-lg font-black text-rose-400">{v.amount.toFixed(2)} ج.م</p>
                                    </div>
            ...
            <Modal isOpen={modalOpen} onClose={closeModal} title={editTarget ? `تعديل سند صرف: ${editTarget.voucherNumber}` : 'إضافة سند صرف جديد'}>
            ...
            <PaymentVoucherDetailsModal voucher={detailsTarget} onClose={() => setDetailsTarget(null)} />

            <Modal isOpen={!!confirmTarget} onClose={() => setConfirmTarget(null)} title="تأكيد سند الصرف">
                <div className="space-y-4">
                    <p className="text-slate-300">
                        هل أنت متأكد من تأكيد سند الصرف رقم <span className="font-bold text-white">"{confirmTarget?.voucherNumber}"</span>؟
                    </p>
                    <p className="text-xs text-slate-400 bg-slate-950 p-3 rounded-xl border border-slate-800">
                        سيتم خصم المبلغ من الخزنة (بعد التحقق من كفاية الرصيد) وتحديث حساب الطرف (إن وجد) تلقائياً.
                    </p>
                    <div className="flex gap-3 pt-2">
                        <button
            ...
            <Modal isOpen={!!cancelTarget} onClose={() => setCancelTarget(null)} title="إلغاء سند الصرف">
                <div className="space-y-4">
                    <p className="text-slate-300">
                        هل أنت متأكد من إلغاء سند الصرف رقم <span className="font-bold text-white">"{cancelTarget?.voucherNumber}"</span>؟
                    </p>
                    <p className="text-xs text-amber-400 bg-amber-500/10 p-3 rounded-xl border border-amber-500/20">
                        تحذير: هذا السند مؤكد. إلغاؤه سيؤدي إلى عكس الحركات المالية ورد المبالغ للخزنة تلقائياً عبر قيود العكس.
                    </p>
                    <div className="flex gap-3 pt-2">
                        <button
