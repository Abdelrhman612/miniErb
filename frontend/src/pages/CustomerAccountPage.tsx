...
import type { CustomerAccountResponseDto, AccountTransactionResponseDto } from '../types';
...
export function CustomerAccountPage() {
    ...
    const [account, setAccount] = useState<CustomerAccountResponseDto | null>(null);
    ...
    const loadData = async (fDate?: string, tDate?: string) => {
        try {
            ...
            const [accData, txData] = await Promise.all([
                ...
                customerService.getAccountTransactions(customerId, fDate, tDate),
            ]);
            setAccount(accData);
            setTransactions(txData);
        } catch {
            setError('فشل في تحميل بيانات حساب العميل.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="p-8 max-w-7xl mx-auto print:p-0 print:max-w-none text-right" dir="rtl">
            ...
            <div className="print-container bg-slate-900/60 print:bg-white border border-slate-800 print:border-none rounded-3xl p-6 md:p-8 space-y-6">
                <div className="border-b border-slate-800 print:border-slate-300 pb-6 flex justify-between items-start">
                    <div>
                        <h1 className="text-2xl font-black text-slate-100 print:text-slate-900">كشف حساب عميل</h1>
                    ...
                    <div className="text-left print:text-right">
                        ...
                        <p className="text-xs text-slate-500">إدارة الحسابات والمبيعات</p>
                    </div>
                </div>
                ...
                {account && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-slate-950/80 print:bg-slate-100 p-4 rounded-2xl border border-slate-800 print:border-slate-300">
                            <span className="text-xs text-slate-400 print:text-slate-600 block">العميل</span>
                            <span className="text-base font-bold text-slate-100 print:text-slate-900 block mt-1">{account.customerName}</span>
                        ...
                        <div className="bg-slate-950/80 print:bg-slate-100 p-4 rounded-2xl border border-slate-800 print:border-slate-300">
                            ...
                            <span className="text-xs text-slate-500 block mt-1">إجمالي المبيعات: {account.totalSales.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                        </div>
                        ...
                        <div className="bg-emerald-500/10 print:bg-emerald-50 p-4 rounded-2xl border border-emerald-500/20 print:border-emerald-300">
                            <span className="text-xs text-emerald-400 print:text-emerald-800 block font-semibold">الرصيد الحالي</span>
